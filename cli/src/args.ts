...
export function parseArgs(argv: readonly string[]): ParsedArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--pm":
        out.packageManager = parsePackageManager(expectValue("--package-manager", argv[++i]));
        break;
      default: {
        if (arg.startsWith("--template=")) {
          ...
          out.packageManager = parsePackageManager(arg.slice("--package-manager=".length));
        } else if (arg.startsWith("--pm=")) {
          out.packageManager = parsePackageManager(arg.slice("--pm=".length));
        } else if (arg.startsWith("-t=")) {
          out.template = arg.slice("-t=".length);
...
function parsePackageManager(value: string): PackageManager {
  ...
  throw new ArgError(`Invalid --package-manager "${value}". Expected one of: ${VALID_PM.join(", ")}.`);
}
...
function parseNetwork(value: string): Network {
  ...
  throw new ArgError(`Invalid --network "${value}". Expected one of: ${VALID_NETWORK.join(", ")}.`);
}

/** The `--help` text. */
export function helpText(): string {
  const templates = TEMPLATES.map((t) => `    ${t.id.padEnd(18)} ${t.hint}`).join("\n");
  return `create-qorechain-rollup — scaffold a new QoreChain rollup project.
