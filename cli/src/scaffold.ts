 ...
 * (plus an optional `spawn` for install).
 */
...
export interface ScaffoldOptions {
  ...
  /** Monorepo root used to compute `file:` links when `local` is set. */
  monorepoRoot?: string;
}
...
const KNOWN_TEMPLATE = "defi-rollup";

/**
 * Locate the bundled `templates/` directory. Works from source (templates at the
 * repo root) and from the built/published package (`<pkg>/templates`).
 */
export function resolveTemplatesRoot(): string {
  const bundled = resolve(here, "..", "templates");
  if (existsSync(join(bundled, KNOWN_TEMPLATE))) return bundled;
  const sourceLevel = resolve(here, "..", "..", "templates");
  if (existsSync(join(sourceLevel, KNOWN_TEMPLATE))) return sourceLevel;
  return bundled;
}

/** Locate the monorepo root (which contains `packages/`), for `--local` links. */
...
function rewriteLocalDeps(pkg: Record<string, unknown>, monorepoRoot: string): void {
  const localPackages: Record<string, string> = {
    "@qorechain/rdk": join(monorepoRoot, "packages", "ts"),
  };
...
export function scaffold(options: ScaffoldOptions): ScaffoldResult {
  ...
  if (!info) {
    throw new Error(`Unknown template "${template}". Available templates: ${templateIdList()}.`);
  }
  ...
  if (!existsSync(templateDir)) {
    throw new Error(
      `Template "${info.id}" not found on disk at ${templateDir}. The package may be built incorrectly.`,
    );
  }
